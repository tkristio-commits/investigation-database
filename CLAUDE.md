...
| `executive-summary.html` | Standalone bilingual (Bahasa / English toggle) executive summary for IPI members. Linked from the `index.html` masthead ("Executive summary →"). Hand-written prose + hard-coded figures — **not** regenerated by `build-data.ps1`; update its numbers by hand when the dataset changes. Also published as a Claude artifact (same file, kept in sync from `../Document/Ringkasan-Eksekutif-IPI.html`). |
